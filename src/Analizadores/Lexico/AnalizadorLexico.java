...
public class AnalizadorLexico {
    ...
    public boolean operadoresArit(String cadena, int linea) {
        ...
        if(isComentario(cadena) || !cadena.matches("[+\\-*/%]|:=")) return false;
        int size = cadena.length();
        for (int i = 0; i < size; i++) {
            char operador = cadena.charAt(i);
            switch (operador) {
                case '*' -> writeToFile("*", -21, linea);
                case '/' -> writeToFile("/", -22, linea);
                case '%' -> writeToFile("%", -23, linea);
                case '+' -> writeToFile("+", -24, linea);
                case '-' -> writeToFile("-", -25, linea);
                case ':' -> {
